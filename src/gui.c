...
static void on_execute_button_clicked(GtkWidget * widget, gpointer user_data)
{
    MyWidgets * widgets;
    char * output;
    char * command;
    ...
    command = (char *)gtk_text_buffer_get_text(GTK_TEXT_BUFFER(buffer), &start,
                                               &end, TRUE);
    proc = process_command(command);
    /* command = (char *)gtk_text_view_get_text (GTK_TEXT_VIEW
     * (widgets->textview_in));*/
...
static void on_checkbutton_ll_toggled(GtkWidget * widget, gpointer user_data)
{
    MyWidgets * widgets;
    widgets = (MyWidgets *)user_data;
    if (fl)
...
static void on_spinbutton_ll_value_changed(GtkWidget * widget,
                                           gpointer user_data)
{
    MyWidgets * widgets;
    widgets = (MyWidgets *)user_data;
...
static void on_help_button_clicked(GtkWidget * widget, gpointer user_data)
{
    MyWidgets * widgets;
    GtkTextBuffer * buffer;
    widgets = (MyWidgets *)user_data;
...
int main(int argc, char * argv[])
{
    ...
    /* build GladeXML object from the glade XML file which we compiled in
     * through gui.h */
    ...
    widgets->toggle =
        GTK_WIDGET(gtk_builder_get_object(gxml, "checkbutton_ll"));
    ...
    */
    g_signal_connect(gtk_builder_get_object(gxml, "execute_button"), "clicked",
                     G_CALLBACK(on_execute_button_clicked), widgets);
    g_signal_connect(gtk_builder_get_object(gxml, "help_button"), "clicked",
                     G_CALLBACK(on_help_button_clicked), widgets);
    g_signal_connect(gtk_builder_get_object(gxml, "checkbutton_ll"), "toggled",
                     G_CALLBACK(on_checkbutton_ll_toggled), widgets);
    g_signal_connect(gtk_builder_get_object(gxml, "spinbutton_ll"),
                     "value_changed",
                     G_CALLBACK(on_spinbutton_ll_value_changed), widgets);
